...
import { MenuController, AlertController, LoadingController, ModalController, IonToggle } from '@ionic/angular';
...
export class RegistrarPage implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.updateusuario1 = this.formBuilder.group({
      es_empresa: ['0', ],
      tipo_documento: ['1', ],
      numero_documento: ['', [Validators.required, Validators.minLength(7), Validators.maxLength(16), Validators.pattern('^[0-9]+$')]],
  ...
  consultarempresa(event){
    if(event.target.value.length>10&&event.target.value>1000000000&&event.target.value<99000000000){
      ...
      this.varios.variasfunciones(datadollarplusconsultarruc).subscribe(async (res: any) => {
  ...
  consultardnioruc(event){
    if(this.updateusuario1.value.tipo_documento=='1'&&event.target.value.length>7&&event.target.value>1000000&&event.target.value<99000000){
      ...
      this.varios.MostrarYOcultarAlertaMono('present');
      this.varios.variasfunciones(datadollarplusconsultardni).subscribe(async (res: any) => {
      ...
      else if(this.updateusuario1.value.tipo_documento=='5'&&event.target.value.length>10&&event.target.value>1000000000&&event.target.value<99000000000){
...
IONCHANGEdistrito(event){
  ...
  async consoledeerrores(){
    ...
    console.log('el boton se presion: formulario invalido, ya se imprimio la consola de erroes', )
  ...
  if(this.acepto_terminos){
    ...
    if (this.registrousuario.valid && this.registrousuario.value.password == this.registrousuario.value.password_repetir) {
      var date = new Date();
      ...
      console.log('el usuario intenta registrarse con esta data,', datadollarplusappcreateuser);
      this.varios.MostrarYOcultarAlertaMono('present');
      this.varios.variasfunciones(datadollarplusappcreateuser).subscribe(async (res: any) => {
        this.varios.MostrarYOcultarAlertaMono('dismiss');
        ...
        if (res&&res.id > 0) {
          ...
          this.varios.presentToast("..::Usuario Registrado correctamente::..");
        ...
        error => {
          ...
          if (error) {
            this.varios.loading2segundos("El usuario o su correo ya fue registrado intente con otro.");
            this.varios.presentToast("..::intente con otro Usuario/Correo::..");
  ...
  enviarregistrocompletadoFULListo(){
    ...
    if(this.updateusuario2.valid){
      ...
      const resultadodebusqudadepartamento = this.departamentos.find(({ id_ubigeo }) => id_ubigeo === this.updateusuario2.value.region);
      const resultadodebusqudaprovincia = ProvinciasService.provincias[this.updateusuario2.value.region].find(({ id_ubigeo }) => id_ubigeo === this.updateusuario2.value.provincia);
      const resultadodebusqudadistrito = DistritosService.distritos[this.updateusuario2.value.provincia].find(({ id_ubigeo }) => id_ubigeo === this.updateusuario2.value.distrito);
      ...
      console.log('datadollarplusappcompletandoregistro:', datadollarplusappcompletandoregistro);
      this.varios.MostrarYOcultarAlertaMono('present');
      this.varios.variasfunciones(datadollarplusappcompletandoregistro).subscribe(async (res: any) => {
        this.varios.MostrarYOcultarAlertaMono('dismiss');
  ...
  async abrir_modal_politico(){
    ...
    modal.onDidDismiss().then((data) => {
      ...
      if(data.data.nombre_politico&&data.data.institucion_politica&&data.data.cargo_politico){
